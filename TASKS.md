...
| 038l | Fix multi-product split over-splitting (hyphen-continuation)  | ✅     | OpenCode (splitOfferHeading now refuses to split when the pre-"eller" token ends in `-` — "Softkerne- eller græskarkernerugbrød" stays one product; wrong 038j test rewritten, genuine splits still pass, 8 split tests; repairOverSplitOffers re-pointed 110 false-split offers to restored whole products and deleted 92 phantom fragments — idempotent re-run 0/0; zero hyphen-fragments remain, genuine Coca-Cola/Fanta/Tuborg splits intact; 141 tests). **Deploy: run repairOverSplitOffers on the box (Nick)**                                                     | 038j (split)                              |
| 038m | Add measurement (grams/liters) to products                    | 🔄     | **Found 2026-08-15** (Nick): Coca-Cola, NUTELLA etc. should show a measurement (1,5 L / 400 g). Root cause (Ultron): `product.unit` + `product.size_grams` exist but are ALWAYS null — the offer already carries the measurement (`quantity.unit.symbol` + `quantity.size.from/to`, verified in fixtures: g/470-1080 bread, ml/750 is) but ingestion hardcodes null (tjek-ingest.ts:38). Fix: propagate unit+size from offer to product, display on product page, keep unit-price comparison honest. Spec: `tasks/038m-product-measurements.md`. **NOT yet implemented.** | Phase 2 product schema                    |
