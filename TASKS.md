...
| 038d | Fix mobile overflow (lists "Opret liste" + sweep)  | 🔄     | **Found 2026-08-15** (Nick): "Opret liste" button overflows viewport on mobile. Root cause: `/lists` "Eller start en tom liste" form is a single `flex items-center` row — `flex-1` input shrinks, select + button keep intrinsic width and push off-screen. Fix: form wraps/stacks on mobile (`flex-wrap` or `flex-col sm:flex-row`); sweep other pages for the same bug class (lists detail/import, upload, report, compare, signin); no overflow-x-hidden masking. Spec: `tasks/038d-mobile-overflow-fixes.md`. **NOT yet implemented.** | 038c (mobile nav); beta usability   |
