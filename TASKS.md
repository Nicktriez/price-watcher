...
| 037e | Replace email magic-link with Passkey (WebAuthn) sign-in | 🔄     | Spec written (2026-08-15): `passkey_credential` table, `@simplewebauthn/server`+`browser`, start/finish registration+auth, counter-increase check, DB-stored challenges, attestation "none", no recovery fallback (beta decision), privacy-policy update. **NOT yet implemented** — DECIDED (Nick): email OTP never had a delivery path (dev-only console log), passkey removes the problem. **HTTPS is a hard prerequisite — beta.skujeg.dk now serves TLS, unblocking this.** | 010 (replace OTP), **HTTPS deployed 2026-08-15**                                                                                                                   |
