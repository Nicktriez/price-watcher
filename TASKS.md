...
| 038j | Split multi-product offers into separate products             | 🔄     | **Found 2026-08-15** (Nick): "Coca-Cola, Coca-Cola Zero eller Fanta" appears as ONE selectable product on lists page. Root cause (Ultron): NOT a matching bug — Danish offers sell several alternatives per deal ("Coca-Cola, Fanta eller Tuborg Squash 24-pak — 69 kr"), and ingestion creates ONE product per heading. **28% of catalog (744/2657) has "eller"**. Decision (Nick): Option B — split multi-product offers at ingestion into separate products, each sharing the deal price; backfill existing 744; keep idempotency + cross-chain matching. Spec: `tasks/038j-split-multi-product-offers.md`. **NOT yet implemented.** | Phase 2 ingestion; 008 matching           |
| 038k | Add product search to offers page                             | 🔄     | **Found 2026-08-15** (Nick): `/offers` has chain filter + pagination but no text search by product name. Add a `?q=` search input (server-side, URL-addressable, composes with chain filter, pagination works within search). Different from existing `searchProducts` (that's list-add/report product picking; this searches offers with prices). Composes with 038j once landed. Spec: `tasks/038k-offers-search.md`. **NOT yet implemented.**                                                                                                                                                                                        | Phase 4; composes with 038j               |
