...
| 038b | Fix passkey rpId/origin behind reverse proxy       | 🔄     | **Bug found 2026-08-15** (beta deploy): server sends `rpId: 127.0.0.1` behind nginx (reads internal `event.request.url`), browser rejects every ceremony, both register+login silently fail. Fix: `rpInfo()` must prefer `process.env.ORIGIN` when set; box `.env` gets `ORIGIN=https://beta.skujeg.dk`. Spec: `tasks/038b-passkey-rpinfo-origin.md`. **NOT yet implemented.**                                                                                                                                                | 037e (deployed, broken in prod) |
