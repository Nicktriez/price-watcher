...
| 038g | Compare page: don't silently drop free-text list items        | 🔄     | **Found 2026-08-15** (Nick): items added via "Eller tilføj fritekst" don't show on compare. Root cause (Ultron): `getBasketCosts` filters `product_id IS NOT NULL`, dropping all free-text items before pricing. Fix (layer 1 only, decided): remove the filter so free-text items flow as no-price + count toward unpriced; honest Danish message when list is all-free-text (can't compare, no product link) vs generic "no stores have prices". NO free-text→product matching (separate feature). Spec: `tasks/038g-free-text-compare.md`. **NOT yet implemented.** | 021 (basket math); beta usability         |
