...
| 038m | Add measurement (grams/liters) to products                    | ✅     | OpenCode (migration 0019 renames size_grams->size + adds size_to; offerMeasurement propagates unit/size/min from offer.quantity; upsertProduct updates via COALESCE; backfillProductMeasurements set measurements on 5592 products; product page shows "350 g"/"1,5 l" + per-offer size & kr/l unit price; offers page cards show size + unit price; stk/null stay without measurement; ranges use min + show "– max"; 141 tests). **Deploy: run backfillProductMeasurements on the box (Nick)**                              | Phase 2 product schema                    |
