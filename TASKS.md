...
| 038b | Fix passkey rpId/origin behind reverse proxy       | ✅     | OpenCode (code fixed: `rpInfo()` prefers `process.env.ORIGIN` when set; verified via dev server RPC that `ORIGIN=https://beta.skujeg.dk` -> rpId beta.skujeg.dk and unset -> localhost; no hardcoded domain). **Deploy verification on the box still pending (Nick): set ORIGIN in .env + confirm register/login completes**                                                                                                                                                                                                  | 037e (deployed, broken in prod) |
