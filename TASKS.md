...
| 030b | Wire free-text crowd reports into Community tiering | ⬜     | OpenCode or Nick — 030 recorded decision (a) but only product-linked path wired; `normalizeProductName` is test-only. Group `product_id IS NULL` by normalized name per store, run through `computeCrowdTier`, surface visibly. Task file: `tasks/030b-free-text-grouping.md`              | 030                   |
