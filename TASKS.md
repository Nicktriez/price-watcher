...
| 038h | Compare page: tell users fuzzy matching isn't available yet   | 🔄     | **Found 2026-08-15** (Nick): beta users may expect free-text "Coca-Cola" to match a real offer (that's fuzzy matching, Task 047, post-beta, not built). Add a brief plain-Danish note to the compare page's free-text-unpriced empty state that fuzzy recognition isn't available yet (comes later), so it's a set expectation not a cold surprise. Copy-only. Spec: `tasks/038h-fuzzy-match-notice.md`. **NOT yet implemented.**                                                                                             | 038g; plan 047 (post-beta)                |
