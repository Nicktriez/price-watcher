...
| **pm2 / systemd**              | Process manager / service manager                                                                     | Runs the single Nitro server process in production on a **separate Hetzner VPS** (app + receipt worker + offer/fuel scheduler all in it — branch `simplify-deployment`), restarts on crash, keeps it alive                                 |
...
| `src/server/ingest-scheduler.ts` | node-cron job — runs ingest on a ~6h cadence + daily fuel refresh. Starts with the app runtime (see `src/entry-server.tsx`); `node src/server/ingest-scheduler.ts --once` does a one-shot manual run |
...
- **Task 037 — Landing page:** `/` is the landing page for **everyone** (signed in or out) — no reroute. "Forside" → `/` (the landing). Offers live at `/offers` under "Tilbud". Hero + "Sku' jeg?" hook, entry points, "what you get", sign-in affordance. (The earlier "signed-in reroutes to /offers" idea was **reversed 2026-08-14** — a signed-in user clicking Home and getting bounced was jarring.)
