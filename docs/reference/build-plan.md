...
**Status:** Task 010 (identity + receipt schema) ✅ complete. Next: Task 011 (OCR parser) — **gated on Task 016** (classifier refinement), which needs ≥10 receipts with multiple samples per failure mode. Gather more receipts in parallel while 011/016 waits.
...
6. **"Your price vs. average" on every scanned receipt:** immediately after scanning, each line shows whether the user paid above/below the going rate ("coffee 42 kr — 3 kr below average"). This is the payoff that turns the _scanner_ into the beneficiary (not just the donor to the crowd layer). Without it, uploading is a favor to the platform and favors don't scale.
...
1. (User accounts moved to Phase 3 — magic-link identity already exists. Phase 6 can add auth _hardening_ only if actually needed: password reset, social login, roles. Keep it boring; skip unless required.)
