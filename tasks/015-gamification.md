...
- **Points awarded ONCE per physical receipt** — pairs with Task 012's dedup. Re-uploading the same receipt (better/worse/accidental) must NOT re-award points. A better re-scan may *upgrade* the points if the improved parse crosses a confidence threshold, but the user doesn't get the base award twice.
