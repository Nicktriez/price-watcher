# Task 016 — Receipt Gamification (points + streaks)
...
Reward users for uploading receipts — **points per accepted receipt, streak bonuses** — to drive the retention loop that makes receipts compound. Depends on identity (Task 010) and the upload flow (Task 013).
...
2. **Award logic** — on each successful receipt parse (Task 013):
...
- **Points awarded ONCE per physical receipt** — pairs with Task 013's dedup. Re-uploading the same receipt (better/worse/accidental) must NOT re-award points. A better re-scan may _upgrade_ the points if the improved parse crosses a confidence threshold, but the user doesn't get the base award twice.
