...
Make the core flows **navigable by a non-technical user without being walked through it**:

0. **Sign in via magic-link** (email → link → signed in) — `src/routes/signin.tsx`
...
**IMPORTANT — sign-in is a flow, not a footnote.** All three product flows gate on `getCurrentUser()` + `<Navigate href="/signin" />`. So a cold non-technical user's actual FIRST experience is the **magic-link sign-in**, and if they can't complete that, they can't do anything else. Treat sign-in as flow #0: verify a non-technical user can request the link, get the email, click it, and land signed-in — without help. This is a precondition to the other three, so check it first.
...
1. **Walk the flows as a first-time user** (this is the audit). For each flow, trace: where does a new user land? Is there an obvious entry point (button/link) to start? Can they complete the whole flow without a dead end, confusing label, or missing affordance? **Do sign-in FIRST (flow #0)**, then the product flows.
...
3. **The specific cold-start gaps to check** (common here):
   - **Magic-link sign-in**: can a first-timer request the link, find it in their email, and land signed in? Is the sign-in entry point obvious?
   ...
   - **The compare path starts FROM a list** — `/compare/[id]` needs a list id, so the flow is list → "compare this list" link. Verify that link on the list page (line ~142 of `lists/[id].tsx`) is obvious and correctly placed. There is no top-level "compare" page; don't invent one.
4. **Danish, plain labels** for every affordance (no English leakage — this pairs with Task 045 but must not wait for it).

## Important

- **Blockers only, not polish** — fix what _stops_ a flow. Visual polish is Phase 9 (Task 040), NOT this task. Don't restyle; make it findable and finishable.
...
- [ ] A first-time user can **sign in via magic-link** without help (request link → email → land signed-in) — this gates everything
...
- [ ] A first-time user can **see a store comparison** for a list without help — obvious "compare this list" link on the list page, no dead ends
- [ ] Every entry point / CTA is plain Danish, no English leakage
- [ ] No flow requires knowing a URL by heart (all reachable from home / empty states / CTAs / the list page)
