...
- **Free-text grouping — DECIDED (2026-08-14, option (a)):** `crowd_report.product_id` is nullable (Task 029 free-text fallback), but free-text reports MUST still be able to reach Community. Group by **normalized name (trim + lowercase)** so agreeing free-text reports group together and can flip to Community. Do NOT wait for moderation (032) to link them — that would leave the crowd layer inert until 032 is done. Risk (two spellings not grouping) is acceptable now; 032 can tighten later. Record: free-text reports group by normalized `product_name`; product-linked reports group by `product_id`.
